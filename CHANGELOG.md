...
- New site settings: `Presets` (a tenant's named blocks), `Currency` (what the `money` format uses)
  and `EmbedHosts` (which hosts an `embed` block may frame). A preset that is not used says why once
  in the server log, whether it took a registered block's name or came after the ones that fill the
  block budget. (#33)
